...
6. **Resend (order status emails)** — add `RESEND_API_KEY` and `EMAIL_FROM` (verified domain,
   e.g. `Doron Goldstein Photography <orders@dgwlp.co.za>`). Customers get emails on checkout
   receipt, payment webhooks (`paid`), and every admin status change.
